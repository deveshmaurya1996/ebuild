...
class NinjaBackend:
    ...
    def _write_ninja(self) -> None:
        ...
        # Quote the generating interpreter for the shell Ninja will use, and
        # escape `$` so Ninja does not treat path fragments as variables.
        python_command = (
        ...
        lines = [
            ...
            # the archive. recreate_archive deletes $out first. A -m module
            # keeps this portable (no `rm`) and avoids fragile -c quoting.
            f"  command = {python_command} -m ebuild.build.recreate_archive "
            "$out $ar rcs $out $in",
            "  description = AR $out",
            "",
